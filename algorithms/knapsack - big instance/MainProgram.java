...
public class MainProgram {
    ...
    public static void main(String[] args) {
        ...
        Item[] items = createArrayFromFile("bigdata.txt");
        
        // The weight capacity of the knapsack.
        int knapsackCapacity = 2000000;
    ...
    public static int computeOptimalKnapsackSolutionValue(Item[] items, int knapsackCapacity) {
        ...
        int[][] optimalSolutionValues = new int[2][knapsackCapacity + 1];
        ...
        for (int i = 1; i <= items.length; i++) {
            for (int x = 0; x <= knapsackCapacity; x++) {
                ...
                if (items[i - 1].getWeight() > x) { // Edge case. Ignore Case 2.
                    if (i == 2) {
                        maxValue = optimalSolutionValues[1][x];
                    } else {
                        maxValue = optimalSolutionValues[(i - 1) % 2][x];
                    }
                } else { // Find the max of Case 1, Case2.
                    if (i == 2) {
                        maxValue = Math.max(optimalSolutionValues[1][x], optimalSolutionValues[1][x - items[i - 1].getWeight()] + items[i - 1].getValue());
                    } else {
                        maxValue = Math.max(optimalSolutionValues[(i - 1) % 2][x], optimalSolutionValues[(i - 1) % 2][x - items[i - 1].getWeight()] + items[i - 1].getValue());
                    }
                }
                
                if (i == 1) {
                    optimalSolutionValues[1][x] = maxValue;
                } else {
                    optimalSolutionValues[i % 2][x] = maxValue;
                }
            }
        }
        
        return Math.max(optimalSolutionValues[0][knapsackCapacity], optimalSolutionValues[1][knapsackCapacity]);
    }
