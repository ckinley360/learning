...
try:
	# Create a list to hold the call times, which will be used to calculate summary statistics
	data = list()
	...
	for line in sys.stdin.readlines():
		...
		if (len(pieces) == 10):
			...
			timeDifference = convertedClearedTime - convertedReceivedTime
			timeDifferenceInSeconds = timeDifference.total_seconds()	

			# Add the call time to the list
			data.append(timeDifferenceInSeconds)
		else:
			continue

	# Calculate and output the summary statistics
	average = statistics.mean(data)
	standardDeviation = statistics.stdev(data)
	print(str(average) + "\t" + str(standardDeviation))
